...
def compute_date_status(logfile_status_list, collection, date=None):
    ...
    if collection == 'scl':
        ...
        if date > datetime.datetime.strptime('2025-08-16', '%Y-%m-%d').date():
            expected_status_sum = COLLECTION_TO_EXPECTED_DAILY_STATUS_SUM.get(collection).get('after_2025_08_16')
        elif date > datetime.datetime.strptime('2022-05-08', '%Y-%m-%d').date():
            expected_status_sum = COLLECTION_TO_EXPECTED_DAILY_STATUS_SUM.get(collection).get('after_2022_05_08')
        elif date > datetime.datetime.strptime('2022-04-05', '%Y-%m-%d').date():
